...
def point_sampling(data_all, num_points, fps = True):
    ...
    #torch.save(points_all, 'points_all_fps_3840_verse19train.pth')
...
def train_graph2image(data_path, encoder, decoder,k, num_p, config, save_to):
    ...
    writer = SummaryWriter('runs/graph2imagek40')
    ...
    for run in trange(num_runs):
        ...
        for i in range(iterations):
            ...
            with torch.cuda.amp.autocast():
                    ...
                    x = encoder(points_in.permute(0,2,1),k)
...
def train_graph2point(data_path, encoder, decoder,k, num_p, config, save_to):
    ...
    writer = SummaryWriter('runs/graph2point')
    ...
    for run in trange(num_runs):
        ...
        for i in range(iterations):
            ...
            if ((i + 1) % B_acc == 0) or (i + 1 == iterations):
                ...
                optimizer.zero_grad()
                if i > 4:
                    writer.add_scalar('loss/Chamfer/train/', run_loss[i-4:i].mean(), int(i//4))
                    writer.add_scalar('loss/Chamfer/val', run_loss_val[i-4:i].mean(), int(i//4))
                    writer.add_scalar('loss/Chamfer/train-val', run_loss[i-4:i].mean() - run_loss_val[i-4:i].mean(), int(i//4))
...
def train_mlp_graph2fx(path, enc, k, mlp, config, save_to, data_ratio=1, no_mild = False, ratio = False):
    num_iterations = 6000
    ...
    if ratio:
        ...
        writer_n = 'runs_mlp/data_split_graph2pb_r' + str(data_ratio)
        writer = SummaryWriter(writer_n)
        num_iterations = 6000
    else:
        writer = SummaryWriter('runs_mlp/graph2img')
    ...
    with tqdm(total=num_iterations, file=sys.stdout) as pbar:
        ...
        for i in range(num_iterations):
            ...
            writer.add_scalar('loss/no_mild/train/', run_loss[i].mean(), i)
        ...
        torch.save(mlp, 'run_checkpoint_graph20_'+ save_to+ str(seed) +'_mlp.pth')
        t_run2 = time.time()
